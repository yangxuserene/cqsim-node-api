...
class Node_struc:
    def __init__(self, debug=None):
        ...
        self.Hilbert3d_location= []
        self.Rowcol_location = []
...
#         print "--------", result_list ,"-------------"
    ...
    def import_node_file(self, node_file):
        ...
        self.Hilbert3d_location = 4096*[-1]#4096==16*16*16
        ...
        while (1):
            ...
            tempInfo = {"id": int(temp_dataList[0]), \
                          "location": map(int, self.read_list(temp_dataList[1])), \
            ...
            '''This part is for coords to d_index'''
            temp_location = tempInfo['location']
            d_index = i
            self.Rowcol_location.append(temp_location)
            self.nodePool.append(d_index)
            '''This part is for coords to d_index'''
            
            
            
            '''This part is for coords to h_index'''
#             temp_location = tempInfo['location']
#             h_index = Hilbert3D.Hilbert_to_int(temp_location)
#             self.Hilbert3d_location[h_index] = temp_location
#             self.nodePool.append(h_index)
            '''This part is for coords to h_index'''
        ...
        self.idle = self.tot
        self.avail = self.tot
        self.debug.debug("  Tot:"+str(self.tot)+\
                         " Idle:"+str(self.idle)+\
                         " Avail:"+str(self.avail)+" ",4)
        return
    ...
    def import_node_config (self, config_file):
        ...
        '''
        Read node configuration info from XXX_node.con file
...
#         self.avail = self.tot
    ...
    def node_allocate(self, node_req, job_index, start, end):
        ...
        allocated_list = temp_node        

        
#         metric = float(Dis_Metric.avg_pairwise_dis_Hilbert3D(allocated_list, self.Hilbert3d_location))
        metric = float(Dis_Metric.avg_pairwise_dis_DimenOrder3D(allocated_list, self.Rowcol_location))      

        
#         if(len(allocated_list)>=8):
        if(len(allocated_list)>=4&len(allocated_list)<=32):
#             self.set_hilbert_penalty(metric)
            self.set_dimensional_penalty(metric)      
               
        reqTime = end-start
        reqTime *= self.get_penalty()
        end = start+ reqTime
#         print reqTime,"-------"
    ...
    def set_dimensional_penalty(self, metric):
        self.penalty = 1.0
        self.penalty += float(metric/10)*1.5
   
#     def set_hilbert_penalty(self, metric):
#         self.penalty = 1.0
#         self.penalty += float(metric/10)*1.5
#         print "*****", self.penalty
    ...
    def recover_place(self,node_list):
        self.nodePool[0:0]=node_list[:]# append node_list at the front of nodePool
